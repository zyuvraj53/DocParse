...
export default function ResumeParser() {
        ...
        if (
          ...
          try {
            const response = await fetch(
              `http://localhost:8000/resumes/${candidate.id}`
            );
            if (response.ok) {
              const data = await response.json();
              details.push({
                ...data,
                total_fit_score: candidate.total_fit_score,
              });
            } else {
              console.warn(
                `Failed to fetch resume ${candidate.id}: ${response.statusText}`
              );
            }
          } catch (err) {
  ...
  const uploadFiles = async () => {
    ...
    try {
      const response = await fetch(
        "http://localhost:8000/uploads/upload-resumes",
        {
          method: "POST",
          body: formData,
        }
    ...
    try {
      ...
      for (const file of uploadedFiles) {
        ...
        const processResponse = await fetch(
          "http://localhost:8000/uploads/process-resumes",
          {
            method: "POST",
            body: formData,
          }
        ...
        if (processResponse.ok) {
          ...
          const validResumes = processResult.resumes.filter(
            resume => !resume.error
          );
          ...
          for (const resume of validResumes) {
            ...
            try {
              ...
              if (resumeResponse.ok) {
                ...
                console.warn(
                  `Failed to post resume ${resume.filename} to /resumes/: ${resumeResponse.statusText}`
                );
                setError(prev =>
                  prev
                    ? `${prev}\nFailed to save resume ${resume.filename}`
                    : `Failed to save resume ${resume.filename}`
                );
                results.push({
                  ...resume,
              ...
              setError(prev =>
                prev
                  ? `${prev}\nError saving resume ${resume.filename}`
                  : `Error saving resume ${resume.filename}`
              );
              results.push({
                ...resume,
          ...
          setError(
            `Processing failed for ${file.name}: ${processResponse.statusText}`
          );
        }
      }
  ...
  const spiderChartData = {
    ...
    datasets: candidateDetails.map((candidate, index) => ({
      ...
      backgroundColor: `rgba(${75 + index * 50}, ${
        192 - index * 50
      }, ${192}, 0.2)`,
  ...
  const barChartData = {
    labels: candidateDetails.map(
      c => c.personal_information?.name || `Candidate ${c.id}`
    ),
    datasets: [
      {
    ...
    <div className="Layout">
      ...
      <div>
        ...
        <div style={{ margin: "1vw", textAlign: "center" }}>
          <label>
            ...
            <textarea
              ...
              placeholder="Enter the job description to compare resumes against..."
            />
          </label>
        </div>
        {/* Dropzone */}
        ...
        {processedResumes.length > 0 && (
          <div style={{ margin: "1vw", textAlign: "center" }}>
            <label>
              Select number of candidates to display (Max:{" "}
        ...
        {candidateDetails.length > 0 && (
          <div style={{ margin: "2vw" }}>
            {/* Spider Chart */}
            <div
              style={{ marginBottom: "2vw", maxWidth: "600px", margin: "auto" }}
            >
              ...
              <Bar
                ...
                options={{
                  ...
                  plugins: {
                    ...
                    title: {
                      display: true,
                      text: "Candidate Skills Breakdown",
                    },
                  },
                }}
              />
